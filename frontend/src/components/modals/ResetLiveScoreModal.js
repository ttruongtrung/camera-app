...
const ResetLiveScoreModal = ({ isOpen, onClose, onSubmit }) => {
  const handleSubmit = () => {
    onSubmit();
    onClose();
  };
      ...
      <div className="w-[700px] bg-white rounded-lg shadow-lg px-6 py-10 z-10">
        <div className="justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-center">
            Bạn có muốn reset bảng điểm không?
          </h2>
          <div className="flex gap-6 justify-center p-6">
            <button
              className="rounded border py-3 w-32 text-center hover:opacity-70 transition"
              onClick={onClose}
            >
              Không
            </button>
            <button
              className="rounded border py-3 w-32 bg-red-500 text-white text-center hover:opacity-70 transition"
...
export default ResetLiveScoreModal;
