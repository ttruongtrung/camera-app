...
const CameraList = ({ cameras, onAddCamera, onSelectCamera, selectedCameraId }) => {
  return (
    <div className="container mx-auto w-full bg-white p-6 rounded-lg overflow-hidden">
      ...
      <div className="flex flex-col gap-4 justify-center items-center">
        {cameras.map(camera => (
          <CameraPanel key={camera.id} camera={camera} onClick={() => onSelectCamera(camera.id)} isSelected={camera.id === selectedCameraId}/>
        ))}
        <div 
